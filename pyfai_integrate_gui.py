...
- Save the current plots, grouped batch videos, and integrated ASCII ``.dat`` files.
...
class DetectorSumWorker(qt.QObject):
    progress = qt.Signal(int, object, int)
    finished = qt.Signal(object, int)
    failed = qt.Signal(str, int)

    def __init__(self, sources, poni, user_mask, generation):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            for index, source in enumerate(self.sources):
                ...
                values.append(masked_intensity_sum(image, mask))
                self.progress.emit(index, values[-1], self.generation)
                del image, mask, detector_mask
            self.finished.emit(np.asarray(values, dtype=np.int64), self.generation)
        except Exception:
...
class MainWindow(qt.QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("pyFAI Integrate Viewer")
    ...
    @qt.Slot(int, object, int)
    def _detector_sum_progress(self, index, value, generation):
        if generation != self._detector_sum_generation:
            return
        self._detector_sum_values = np.append(
            self._detector_sum_values, np.int64(value)
        )
        x = np.arange(1, self._detector_sum_values.size + 1, dtype=np.float64)
        self.detector_sum_plot.addCurve(
            x, self._detector_sum_values, legend="Detector sum intensity",
            symbol="o", resetzoom=True,
        )
        self.status_label.setText(
            f"Calculating detector sum: {index + 1}/{len(self.image_paths)}"
        )
    ...
    def _detector_sum_finished(self, values, generation):
        if generation != self._detector_sum_generation:
            return
        self._detector_sum_values = np.asarray(values, dtype=np.int64)
    ...
    def save_current_view(self):
        ...
        if not self._grab_combined_plots().save(filename):
    ...
    def _grab_combined_plots(self):
        ...
        if self.right_stack.currentWidget() is self.detector_sum_plot:
            right = self.detector_sum_plot.getWidgetHandle().grab().toImage()
    ...
    def _batch_data_finished(self, output_dir, count):
        ...
        if self._combined_export_video_dir is not None:
            ...
            self._start_batch_video_export(video_dir, reset_progress=False)
            return
        self.integrate_button.setEnabled(True)
    ...
    def _start_batch_video_export(self, output_dir, reset_progress=True):
    ...
    def save_data_and_plots(self):
        """Export one ASCII+plot, or all ASCII+videos for multiple sources."""
        if not self.image_paths:
            self.show_error("No Images", "Load one or more images first.")
            return
        poni = self.poni_edit.text().strip()
        ...
        if multiple and not self._select_video_axis_scales():
            return
        output_dir = qt.QFileDialog.getExistingDirectory(
    ...
    def _append_video_frame(self):
        if not self._exporting_video or self._video_writer is None:
            return
